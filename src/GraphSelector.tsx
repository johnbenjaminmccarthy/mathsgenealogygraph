...
export function GraphSelector(props: { presetFunction: (preset: Preset) => void }) {
    ...
    return (
        <>
            <div id={"selector"}>
                <span className={"APIinput"}>
                    <span className={"disabled"}>OUT OF ORDER!!</span>
                    <span id={"genealogyIdText"}>Maths Genealogy URL or ID number:</span>
                    <input type={"text"} id={"genealogyId"}
                           placeholder={"https://www.mathgenealogy.org/id.php?id=74313"}/>
                ...
                <select name={"presets"} id={"presets"} defaultValue={"johnbenjaminmccarthy"} onChange={handlePresetChange}>
                    <option key={Preset.johnbenjaminmccarthy} value={"johnbenjaminmccarthy"}>John McCarthy</option>
                    <option key={Preset.johnbenjaminmccarthybig} value={"johnbenjaminmccarthybig"}>John McCarthy (Big!)</option>
